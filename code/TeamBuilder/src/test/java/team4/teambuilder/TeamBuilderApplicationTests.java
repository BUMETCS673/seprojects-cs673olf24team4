...
class TeamBuilderApplicationTests {
    ...
    @BeforeEach
    public void setUp() {
        // Clear all existing teams first to ensure no foreign key constraints are violated
    ...
    public void testTeamAssignment() {
        ...
        List<Team> teams = teamAssignmentService.assignTeams(testGroup.getId(), 2);
        ...
        assertTrue(teams.get(0).getMembers().size() >= expectedMinTeamSize && teams.get(0).getMembers().size() <= expectedMaxTeamSize,
                "Team 1 size should be between " + expectedMinTeamSize + " and " + expectedMaxTeamSize);
        assertTrue(teams.get(1).getMembers().size() >= expectedMinTeamSize && teams.get(1).getMembers().size() <= expectedMaxTeamSize,
                "Team 2 size should be between " + expectedMinTeamSize + " and " + expectedMaxTeamSize);
        assertEquals(totalUsers, teams.get(0).getMembers().size() + teams.get(1).getMembers().size(),
                "The sum of team sizes should equal the total number of users");

        // Check if roles are distributed across teams
        Set<String> team1Roles = teams.get(0).getMembers().stream().map(User::getRole).collect(Collectors.toSet());
        Set<String> team2Roles = teams.get(1).getMembers().stream().map(User::getRole).collect(Collectors.toSet());
        ...
        assertTrue(
                teams.get(0).getMembers().stream().anyMatch(u -> u.getRole().contains("Design and Implementation Leader")) &&
                        teams.get(1).getMembers().stream().anyMatch(u -> u.getRole().contains("Design and Implementation Leader")),
        ...
        Map<String, Long> team1RoleCounts = teams.get(0).getMembers().stream()
                .collect(Collectors.groupingBy(User::getRole, Collectors.counting()));
        Map<String, Long> team2RoleCounts = teams.get(1).getMembers().stream()
                .collect(Collectors.groupingBy(User::getRole, Collectors.counting()));
        ...
        long team1HighScorers = teams.get(0).getMembers().stream()
                .filter(u -> highScoringUserNames.contains(u.getName()))
                .count();
        long team2HighScorers = teams.get(1).getMembers().stream()
                .filter(u -> highScoringUserNames.contains(u.getName()))
                .count();
        ...
        // Check if teams are saved in the database
        List<Team> savedTeams = teamRepository.findByGroupId(testGroup.getId());
        assertEquals(2, savedTeams.size(), "Two teams should be saved in the database");
        assertEquals(teams.size(), savedTeams.size(), "The number of returned teams should match the number of saved teams");
